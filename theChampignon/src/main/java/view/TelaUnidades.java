...
public class TelaUnidades extends javax.swing.JFrame{
    ...
    public TelaUnidades(JFrame callingFrame) {
        ...
        tab_unidades.setModel(tableModel);
        tableModel.addColumn("ID");
        tableModel.addColumn("Descrição");
       
        tab_unidades.getSelectionModel().addListSelectionListener(evt -> registroSelecionado(evt));
        listarUnidades();
 
        scrollpanel_tabela.setViewportView(tab_unidades);
    ...
    private void btn_editarPressionado(java.awt.event.ActionEvent evt) {
        if (registroSelecionado >= 0) {
            ...
            EdtUnidades telaEditar = new EdtUnidades(this, id, descricao);
    ...
    private void btn_excluirPressionado(java.awt.event.ActionEvent evt) {
        if (registroSelecionado >= 0) {
            long idUnidade = (long) tableModel.getValueAt(registroSelecionado, 0);

            UnidadesRN unidadesRN = new UnidadesRN();
            unidadesRN.excluirUnidade(idUnidade);
    ...
    private void registroSelecionado(ListSelectionEvent evt) {
        if (!evt.getValueIsAdjusting()) {
                registroSelecionado = tab_unidades.getSelectedRow();
        }
    }
